...
<div class="explorerdiv">
    <form action="<#ZC_BLOG_HOST#>" method="post" name="creator" enctype="multipart/form-data">
        筛选：<select name="province" onChange = "select()"></select>　
        <select name="city" onChange = "select()"></select>
        <input type="button" name="s1" id="s1" value="筛选">
    </form>
</div>
...
<footer>
    <div class="noBuy">
        <a href="">立即购买</a>
        <small>快去选择你喜爱的书单吧</small>
    </div>
    <!-- <div class="buy">
        <a href="payment-确认订单支付.html">立即购买</a>
        <strong>129<small>元/期</small></strong>
        <span>原价149元</span>
    </div> -->
</footer>
...
<script language="javascript">
    ...
    where[0]= new comefrom("课程等级","教师筛选");
...
<script>
    ...
    $(function() {
        /*$(".chage").click(function() {
            var index = $(".chage").index(this);
            $(".curList ul").find("li").eq(index).addClass("myChange").siblings().removeClass("myChange");
            $(".chage").eq(index).text("已选").parent().parent().parent("li").siblings().find(".chage").text("选择");
            if($(".curList ul").find("li").hasClass("myChange")) {
                $(".noBuy").hide();
                $(".buy").show();
            } else {
                $(".noBuy").show();
        ...
        //$("#s1").click(function() {
            $.ajax({
                type: "get",
                /*async: true,*/
                url: url+"/api/get_select_courses.aspx"+myToken,
                data: {},
                dataType: "json",
                /*jsonp: "callback",*/
                success: function(data) {//请求成功完成后要执行的方法 each循环 使用$.each方法遍历返回的数据date
                    //console.log(data);
                    $(".curList").find("li").remove();
                    $.each(data.courses, function (n, value) {
                        var myPrice = value.price-value.discount;
                        $(".curList").find("ul").append(
                            "<li><dl>"
                            +"<dt>"
                            +"<h2>"+ value.title +"</h2>"
                            +"<span>"
                            + value.description +"<br>"
                            + value.sort +"&nbsp;" + value.level
                            +"</span>"
                            +"<a class='chage'>选择</a>"
                            +"<i>"+ value.id +"</i>"
                            +"</dt>"
                            +"<dd>"
                            +"<p>"
                            +"<strong>"+ value.jx1 +"</strong>"
                            +"<small>甘蓝精选</small>"
                            +"</p>"
                            +"<p>"
                            +"<strong>"+ value.jx2 +"</strong>"
                            +"<small>甘蓝精选</small>"
                            +"</p>"
                            +"<p>"
                            +"<strong>"+ value.jx3 +"</strong>"
                            +"<small>甘蓝精选</small>"
                            +"</p>"
                            +"</dd>"
                            +"</dl></li>"
                        ...
                        // 价格
                        $(".curList").find("ul li").find(".chage").click(function() {
                            var index = $(".chage").index(this);
                            $(".curList ul").find("li").eq(index).addClass("myChange").siblings().removeClass("myChange");
                            $(".chage").eq(index).text("已选").parent().parent().parent("li").siblings().find(".chage").text("选择");
                            var myId = $(".curList").find("ul li").find("i").eq(index).text();
                            console.log(myId);
                            $.ajax({
                              type: "get",
                              ...
                              success: function(data) {
                                ...
                                if($(".curList ul").find("li").hasClass("myChange")) {
                                    ...
                                    $(".buy").remove();
                                    $("footer").append(
                                        "<div class='buy'><a href='payment_querendingdanzhifu.aspx?courseid="+ data.info[0].course_id +"'>立即购买</a><strong>"+ salePrice +"<small>元/期</small></strong><span>原价"+ data.info[0].price +"</span></div>"
                                    );
                                } else {
                                    $(".noBuy").show();
                                    $(".buy").hide();
                                  ...
                                  /*$.each(data, function (n, value) {
                                    ...
                                    var salePrice = value.price - value.discount;
                                    if(myId == value.class_id) {
                                        if($(".curList ul").find("li").hasClass("myChange")) {
                                            $(".noBuy").hide();
                                            $(".buy").remove();
                                            $("footer").append(
                                                "<div class='buy'><a href='payment-确认订单支付.html?class_id="+ value.class_id +"'>立即购买</a><strong>"+ salePrice +"<small>元/期</small></strong><span>原价"+ value.price +"</span></div>"
                                            );
                                        } else {
                                            $(".noBuy").show();
                                            $(".buy").hide();
